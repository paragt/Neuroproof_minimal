...
class RagNode {
  public:
    ...
    unsigned long long total_border_length();
    void compute_border_lengths(std::vector<double>& ret1, Region desired_type);
...
template<typename Region> void RagNode<Region>::compute_border_lengths(std::vector<double>& blengths, Region desired_type)  {
    unsigned long long count = 0;
    std::set<unsigned long long> sorted_lengths;
    size_t nlengths = 2;
    blengths.clear();
    blengths.resize(1+nlengths,0.0);
    ...
    for (RagNode<Region>::edge_iterator iter = this->edge_begin(); iter != this->edge_end(); ++iter) {
        ...
        count += (*iter)->get_size();
	RagNode<Region>* other_node = (*iter)->get_other_node(this);
	if (other_node->get_node_type() == desired_type)
	    sorted_lengths.insert((*iter)->get_size());
    }
    
    blengths[0] = count;
    
    std::set<unsigned long long>::reverse_iterator siter= sorted_lengths.rbegin();
    for(size_t i=1; (i<=nlengths) && (siter != sorted_lengths.rend()); siter++, i++){
	double ratio1 = (*siter)*1.0/count;
	blengths[i] = ratio1;
    }

}

template<typename Region> unsigned long long RagNode<Region>::total_border_length()  {
    unsigned long long count = 0;
    for (RagNode<Region>::edge_iterator iter = this->edge_begin(); iter != this->edge_end(); ++iter) {
        if ((*iter)->is_false_edge()) {
            continue;
