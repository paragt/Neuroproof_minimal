...
class RagEdge {
  public:
    ...
    void mito_boundary_features(std::vector<double>& mfeat);
...
template<typename Region> double RagEdge<Region>::mito_boundary_ratio(){
   ...
   unsigned long long mito_node_border_len = mito_node->total_border_length();		
...
template<typename Region> void RagEdge<Region>::mito_boundary_features(std::vector<double>& mfeat){
   ...
   std::vector<double> mblen;
   std::vector<double> cblen;
   mito_node->compute_border_lengths(mblen, 1);
   other_node->compute_border_lengths(cblen, 2);
   
   mblen[0] = edge_size*1.0/mblen[0]; 
   cblen[0] = edge_size*1.0/cblen[0]; 
   
   mfeat.clear();
//    mfeat.push_back(edge_size);
   mfeat.insert(mfeat.end(), mblen.begin(), mblen.end());
   mfeat.insert(mfeat.end(), cblen.begin(), cblen.end());
